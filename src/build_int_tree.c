...
void split_and_insert_internal_int(int_node** root, int_node* old_node, int left_index, int key, int_node* right) {
    ...
    for (i = 0, j = 0; i < old_node->number_of_keys + 1; i++, j++) {
        if (j == left_index + 1) j++;
        temp_children[j] = old_node->data.children[i];
    }
    ...
    // Inserting first half of data to old node
    for (i = 0; i < split_point - 1; i++) {
        old_node->data.children[i] = temp_children[i];
        old_node->keys[i] = temp_keys[i];
        old_node->number_of_keys++;
    }
    old_node->data.children[i] = temp_children[i];

    int k_prime = temp_keys[split_point - 1];

    int k = i + 1; 
    for (j = 0; k < ORDER; k++, j++) {
        new_node->data.children[j] = temp_children[k];
        new_node->keys[j] = temp_keys[k];
        new_node->number_of_keys++;
    }
    new_node->data.children[j] = temp_children[k];

    // Changing parent in moved nodes

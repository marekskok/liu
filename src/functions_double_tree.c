// Double functions from this file are used by R API functions
// As in build_tree I copied everything from functions_int_tree.c and change to doubles
#include<stdio.h>
#include<stdlib.h>
#include<stdbool.h>
...
void find_indices_double(double_node* root, double key, int_table* result){
    ...
    // It returns struct which is basically int vector of occurences

    // Empty table
    size_t size = 0;
    int flag = 0;
    // Uses function from build_int_tree.c to find leaf
    double_node* start_leaf = find_leaf_double(root, key);
    double_node* leaf = start_leaf;
    ...
    while(leaf){
        for (size_t i=0; i<leaf->number_of_keys; i++){
            if (leaf->keys[i] == key) {
                result->pointer[j++] = leaf->data.row_indices[i];;
            }
            if (leaf->keys[i] > key) {
                flag = 1;
                break;
...
void free_tree_double(double_node* root) {
    ...
    if (!root->is_leaf) {
        for (size_t i = 0; i <= root->number_of_keys; i++) {
            free_tree_double((double_node*)root->data.children[i]);
        }
    }
    free(root);
}
...
int_table find_indices_min_double(double_node* root, double* min){
    ...
    double_node* first_leaf = curr;
    double min_value = curr->keys[0];
    t.size = 1;
...
int_table find_indices_max_double(double_node* root, double* max){
    ...
    // Saving last node
    double_node* last_leaf = curr;

    // Getting max value and counting keys
    double max_value = curr->keys[curr->number_of_keys-1];
    int flag = 0;
    while(curr){
...
dual_int_table* inner_join_double(double_table v, double_node* root, bool left, size_t* out_size){

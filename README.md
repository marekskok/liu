...
df[row_idx, ]
```
**liu_search_range()** - Finds all row indices with keys within a specified numerical range [start, end) in LIU. index. This operation is very efficient due to the B+Tree structure. You can leave the start, the end, or both blank for an unbounded search.
```R

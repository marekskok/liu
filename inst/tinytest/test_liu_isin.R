...
df <- data.frame(
  ...
  word = c("test1", "test2", "test3", "test4", "test5", "test6", "test7")
)
...
expect_error(liu_isin(idx2, as.integer(5)))
liu_free(idx2)

idx3 <- liu_build(df,"word")
expect_identical(liu_isin(idx3, c("test2", "LIU", "test5")), as.logical(c(TRUE,FALSE,TRUE)))
expect_identical(liu_isin(idx3, as.character(c())), as.logical(c()))
expect_error(liu_isin(idx3, as.integer(5)))
liu_free(idx3)
